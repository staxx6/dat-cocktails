...
export class MongoDbConnectionService implements IDbConnection {
  ...
  async updateIngredient(ingredient: Ingredient): Promise<boolean> {
    ...
    await this._getIngredientCollection().replaceOne({id: ingredient.id}, ingredient);
    console.log(`updated ingredient with id: ${ingredient.id}`);
    return true; // TODO
  }

  async createIngredient(ingredient: Ingredient): Promise<number> {
    // Please explain ki result :)
    const result = await this._getIngredientCollection().find({}, {projection: {id: 1}}).sort({id: -1}).limit(1).toArray();
    console.log('result from find: ' + JSON.stringify(result));
    let resultId;
    if (result.length !== 0) {
      resultId = result[0].id + 1;
    ...
    console.log('new ingredient ID: ' + resultId);
    ingredient.id = resultId;
    await this._getIngredientCollection().insertOne(ingredient);
    console.log(`created ingredient: ` + JSON.stringify(ingredient));
    return resultId; // TODO
  }

  async deleteIngredient(id: number): Promise<boolean> {
    await this._getIngredientCollection().deleteOne({id: id});
