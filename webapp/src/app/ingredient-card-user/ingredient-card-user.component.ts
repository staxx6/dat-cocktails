...
@Component({
  ...
  template: `
    <div class="flex flex-wrap mt-4 bg-base-300 rounded-2xl w-full overflow-hidden">
      <figure *ngIf="getIngredientPicture()" class="w-48">
        <img [src]="getIngredientPicture()"/>
      </figure>
      <div class="flex flex-col p-4">
        <h2 class="text-xl mb-4 text-secondary">{{ingredient?.name ?? 'Loading ...'}}</h2>
        <p>
          {{getDescription()}}
        </p>
      </div>
    </div>
    <button class="btn btn-accent m-4" backButton>Zurück</button>
  `
})
